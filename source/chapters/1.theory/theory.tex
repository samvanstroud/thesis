...
\begin{table}[!htbp]
  ...
  \caption{
    ...
    The recently discovered Higgs particle is the only scalar boson in the SM.
  }
  \label{tab:sm_bosons}
\end{table}
%
...
Quantum electrodynamics (QED) is the relativistic quantum theory which describes the interaction between lighted and charged matter.
...
There are no right handed neutrinos in the standard model.\todo{elaborate}
\todo{add parity operator def?}
...
The charge operator $Q$ can be written \todo{put somehwere else?}
%
...
Physical particles are detected in their mass eigenstates $q$, which diagonalise the mass matrix, but interact via the weak in interaction according to their weak eigenstates $\tilde{q}$, which are superpositions of the mass eigenstates.
This feature of the weak sector leads to mixing between different quarks and different leptons.
...
\subsection{Higgs Phenomenology}\label{sec:higgs_pheno}
...
The strength of the coupling between the Higgs and some other particle is proportional to the particle's mass.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.65\linewidth]{chapters/1.theory/figs/plotAll_13tev_BSM_sqrt.pdf}
  \caption{
    Higgs production cross sections as a function of Higgs mass $m_H$ at $\sqrt{s} = \SI{13}{\TeV}$ \cite{deFlorian:2016spz}.
    Uncertainties are shown in the shaded bands.
    At the Higgs mass $m_H = \SI{125}{\GeV}$, Higgs production is dominated by gluon-gluon fussion, vector boson fusion, and associated production with vector bosons.
  }
  \label{fig:higgs_production_xs}
\end{figure}


Since the Higgs couples proportional to mass as already mentioned, decays to heavier particles are favoured.
The branching ratios of different Higgs decay modes are shown as a function of $m_H$ in \cref{fig:higgs_br}.
Approximately \pct{60} of the time the Higgs decays to a pair of \bquarks, the dominant decay mode since the Higgs it not massive enough to decay to a pair for top quarks (recall from \cref{tab:sm_fermions} $m_b = \SI{4.18}{\GeV}$ and $m_t = \SI{173}{\GeV}$).
...
Decays to pairs of vector bosons are via a virtual off shell Higgs only, since the combined vector boson mass is greater than the Higgs mass.
While the \Hgammagamma and $\PH \rightarrow \Zboson \Pphoton$ branching ratios are very small compared with fermionic decay modes (around \pct{0.2} for \Hgammagamma), these decay channels were instrumental in the initial discovery of the Higgs due to the low level of background processes which mimic the final state.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{chapters/1.theory/figs/Higgs_BR_LM.pdf}
  \caption{
    Higgs branching ratios as a function of Higgs mass $m_H$ at $\sqrt{s} = \SI{13}{\TeV}$ \cite{deFlorian:2016spz}.
    Uncertainties are shown in the shaded bands.
    At the Higgs mass $m_H = \SI{125}{\GeV}$, the Higgs predominantly decays to a pair of \bquarks, around \pct{60} of the time.
    The subdominant decay mode is off shell to a pair of \Wboson bosons.
    }
  \label{fig:higgs_br}
\end{figure}

This thesis presents Higgs analyses using events with a Higgs produced in association with vector boson, where the Higgs decays to a pair of \bquarks $(pp \rightarrow VH(bb))$.
The \Hbb decay mode directly probes the Higgs coupling to the second generation fermions, and more specifically to down-type quarks.
This coupling was first observed in 2018 \cite{ATLAS-CONF-2018-036,CMS-HIG-18-016}.
Ongoing work analysing the coupling strengths, in particular in the high energy regime, is the focus of the analysis presented in this thesis in \cref{chap:vhbb_boosted}.
\todo{why interesting}
%\begin{itemize}
