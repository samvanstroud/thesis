...
\todo{include accellerator complex info?}
\begin{table}[!htbp]
...
As shown in \cref{fig:atlas_coord_system}, the \axis{z} points in the forward direction along the beam pipe, while the \axis{x} points from the interaction point to the centre of the LHC ring, and the \axis{y} points upwards.
...
Cross sections are of primary importance to experimental particle physics.
The cross section $\sigma$ for a given process is closely related to the probability of an interaction between two colliding particles, and is analogous to an effective cross-sectional area of the particles.
The cross section itself depends on the transition matrix element, obtained using the Feynman rules of the theory which are derived using QFT, and a phase space integral.
...
\end{equation}
%
The partonic cross section can be calculated, while the parton distribution functions (PDFs) have to be extracted from experimental results. 

\subsubsection{Luminosity}
Total number of proton-proton collisisons $N$ is related to the total $pp$ cross $\sigma$ section by the integrated luminosity $L$, as in
%
\begin{equation}
  N = \sigma L = \sigma \int \mathcal{L} ~\mathrm{d}t .
\end{equation}
%
The instantaneous luminosity $\mathcal{L}$ relates the cross section with the number of collisions per unit time.
...
The \ATLAS\footnote{\textbf{A} \textbf{T}oroidal \textbf{L}JC \textbf{A}pparatu\textbf{S}.} detector is a general-purpose particle detector based at the \LHC.
The detector is made up of several specialised sub-detectors which are arranged concentrically around the nominal interaction point at the centre of the detector, covering nearly the entire solid angle around the collision point.
...
The inner tracking detector is described in \cref{sec:atlas_id}, the electromagnetic and hadronic calorimeters in \cref{sec:atlas_calorimeter}, the muon spectrometer in \cref{sec:muon_spectrometer}, and the trigger in \cref{sec:trigger}
More complete information on the detector can be found in \rcite{PERF-2007-01}, or in the technical design reports (TDRs) of the individual sub-detectors.
...
\begin{figure}[!htpb]
  ...
  \caption{
    ...
    Cutouts to the centre of the detector reveal the different subdetectors which are arranged in layers around the nominal interaction point.
  }
...
The trigger system is comprised of two levels which search for signs of electrons, muons, taus, photons, and jets, as well as events with high total or missing transverse energy.
...
Jets are an aggregate reconstructed object corresponding to a spray of collimated stable particles which results from a decay chain of an unstable progenitor particle.
...
Particle-flow objects integrate information from both the ID and the calorimeters, improving the energy resolution at high transverse momenta and reducing pileup contamination.
...
The radius parameter is allowed to vary with transverse momentum such that a broader cone (up to $R=0.4$) is used for \lowpt track-jets and a narrower cone (down to $R=0.02$) for \highpt track-jets.
...
Electrons candidates are reconstructed by matching PV-compatible inner detector tracks to topological calorimeter clusters.
...
More information on muon reconstruction can be found in \rcite{PERF-2015-10}.

\subsection{Missing Transverse Momentum}\label{sec:missing_Et}
\todo{lifted from paper}
The missing transverse momentum \vETmiss is reconstructed as the negative vector sum of the momenta of leptons and \smallR jets in the event plus a soft term built from additional tracks associated with the primary vertex \cite{PERF-2016-07}.
\SmallR jets used for the \vETmiss reconstruction are required to have $\pt > \SI{20}{\GeV}$.
The magnitude of \vETmiss is referred to as \ETmiss.

To suppress non-collision and multi backgrounds in the 0-lepton channel, an additional track-based missing transverse momentum estimator, \vETmisstrk, is built independently as the negative vector sum of the transverse momenta of all tracks from the primary vertex.