...
As shown in \cref{fig:atlas_coord_system}, the \axis{z} points in the along the direction the beam pipe, while the \axis{x} points from the interaction point to the centre of the LHC ring, and the \axis{y} points upwards.
...
At the centre of the ATLAS detector, bunches of more than $10^{11}$ protons meet at a small crossing angle.
...
For \highpt tracks (e.g. $\pt \approx \SI{100}{\GeV}$) in the central region, $\sigma(1/\pt) \approx \SI{0.4}{\per\TeV}$ corresponding to a relative error of \pct{4}.
...
\end{equation}
%
corresponding to a jet energy resolution of \pct{10} at a jet \pt of approximately \SI{100}{\GeV} \cite{ATLAS-TDR-03}.
...
The transverse momentum resolution has been measured to be approximately \pct{1.7} in the central region for \lowpt muons, increasing to \pct{4} for \highpt muons in the forward regions \cite{Artoni:1953654}.
%
\begin{figure}[!htpb]
  \centering
...
The first is via a geometrical matching in $\DeltaR = \sqrt{(\Delta \eta)^2 + (\Delta \phi)^2}$, where $\Delta \eta$ and $\Delta \phi$ are the differences in pseudorapidity and azimuthal angle between the jet and the object.
...
More information is available in \rcite{PERF-2014-07}.
...
At the medium identification working point, approximately \pct{96} of muons with $\SI{20}{\GeV} < \pt < \SI{100}{\GeV}$ are successfully identified.
