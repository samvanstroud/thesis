...
Since the completion of its construction in 2008, the Large Hadron Collider (\LHC)~\cite{Evans:2008zzb} at \CERN has extended the frontiers of particle physics through significant increases in centre of mass energy and instantaneous luminosity compared with previous collider experiments.
...
The trajectory of a charged particle track is parameterised as a helix which is fully specified using five parameters: $(\dzero, \zzero, \phi, \theta, q/p)$.
...
Trigger system, used to select events for storage, is described in \cref{sec:trigger}.
...
which enables radially extended track reconstruction in the region $|\eta| < 2.0$ and typically provides 33 (38) additional hits in the barrel (end-cap). 
...
The IBL was installed before \runtwo \cite{ATLAS-TDR-19,PIX-2018-001} and lies approximately just \SI{33}{\milli\meter} from the beam axis (the distance varies due to the interleaved structure).
...
Many ATLAS analyses rely on flavour tagging, which is the identification of jets containing heavy flavour (HF) hadrons (\bhadrons and \chadrons) as opposed to those containing only light-flavour hadrons or gluons (collectively referred to as \ljets).
