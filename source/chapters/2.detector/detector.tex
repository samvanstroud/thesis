...
The reconstructed trajectory of a charged particle (called a \textit{track}) is parameterised as a helix which is fully specified using five parameters: $(\dzero, \zzero, \phi, \theta, q/p)$.
...
Impact parameter significances are defined as the IP divided by its corresponding uncertainty $\sigma$, as in $\dzerosig = d_0 / \dzerouncert$ and $\zzerosig = z_0 / \zzerouncert$.
...
The signed IP significances are positive if the track crosses the jet axis in front of the primary vertex and negative if the track crosses behind the primary vertex.
...
The uncertainty on the total integrated luminosity is \pct{1.7} \cite{ATLAS-CONF-2019-021} as obtained by the LUCID-2 detector \cite{Avoni:2633501}.
%
\begin{figure}[!htbp]
  \centering
...
Tracks are reconstructed from space-points obtained from clusters of charge depositions in the ID subsystems.
...
The detector is required to have a high granularity (i.e. resolution) to maintain the low occupancy required to resolve the trajectories of nearby particles. %(high sparsity to resolve different tracks).
...
\end{equation}
%
The sampling term contributes approximately $\pct{2.8} / \sqrt{E}$, the noise term contributes $\SI{120}{\MeV} / {E}$, while the constant term contributes an additional \pct{0.3} and is dominant at \highpt.
...
The energy resolution for electrons with $\pt = \SI{10}{\GeV}$ varies between \pct{0.3} and \pct{0.8} over the pseudorapidity range.
Meanwhile for photons with $\pt = \SI{60}{\GeV}$ the energy resolution varies between \pct{0.25} and \pct{1} \cite{PERF-2017-03}.
...
\end{equation}
%
This corresponds to an energy resolution of approximately \pct{17} (\pct{6}) for a hadron with \ET of approximately \SI{10}{\GeV} (\SI{100}{\GeV}).


%The jet energy resolution is approximately \pct{10} (\pct{3.5}) for a jet with $\SI{100}{\GeV}$ (\SI{1}{\TeV}) of transverse momentum \cite{Klimek:2020onx}.
%
...
The transverse momentum resolution (measured for combined ID and muon tracks, see \cref{sec:lepton_reco}) has been measured to be approximately \pct{1.7} in the central region for muons from \Jpsi decays, increasing to \pct{2.3} for muons in the forward regions \cite{PERF-2015-10}.
%
\begin{figure}[!htpb]
  \centering
...
Groups of neighbouring pixels or strips are clustered together and space-point coordinates are estimated using the shape and energy distribution of the clusters.
...
In simulation, jet flavour labels are assigned to \smallR jets according to the presence of a generator level hadron within ${\DeltaR(\text{hadron},\text{jet})<0.3}$ of the jet axis. If a \bhadron is found the jet is labelled a \bjet. In the absence of a \bhadron, if a \chadron is found the jet is called a \cjet.
...
After reconstruction, muon candidates undergo an identification processes which helps to efficiently identify prompt muons whilst rejecting background signals (e.g. non-prompt muons from pion, kaon, semi-leptonic heavy flavour hadron decays, and the punch-through of a hadron from the calorimeter).
...
The \text{efficiency} of a flavour tagging algorithm is defined as the fraction of signal jets which are correctly identified by the tagger.
...
Additionally, as the \borchadron decays semileptonically in approximately \pct{40} of cases \cite{Workman:2022ynf} the presence of a reconstructed electron or muon inside a jet provides information about the jet flavour.
...
% TODO integrate with above and make it smoother, it's a bit repeated
...
At the \pct{70} efficiency working point, \DLr achieves a \crej of approximately 12 and a \lrej of approximately 625.
...
%is a deep neural network which accepts as inputs the outputs of a number of independently optimised \textit{low-level} algorithms \cite{FTAG-2018-01}, as shown in \cref{fig:high_low_taggers}.
