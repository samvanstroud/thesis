...
Electrons and muons are reconstructed as outlined in \cref{sec:lepton_reco}, and following the approach described in \rcite{HIGG-2018-04}.
...
Experimental uncertainties arise due to the imperfect reconstruction algorithms (in particular the jet reconstruction and \btagging algorithms), and due to the imperfect modelling of pile-up and other effects, as described in \cref{sec:sources_of_experimental_uncertainties}.
...
\subsection{Sources of Experimental Uncertainties}\label{sec:sources_of_experimental_uncertainties}

The main experimental uncertainties in the analysis are due to the following sources:

\begin{itemize}
  \item The \smallR jet energy scale and resolution, which are informed by in situ\todo{what does in situ mean in this context} calibration studies and the dependence of the jet energy on the level of pile-up \cite{PERF-2016-04}.
  ...
  \item Uncertainties associated with the reconstruction of the missing transverse energy \ETmiss, which have various sources as described in \rcite{PERF-2016-07}.
\end{itemize}
...
The 1-lepton channel was used as it has the largest amount of available statistics.
As far as was possible given the limited statistics available in the other channels, compatability was checked between the derived uncertainties and the other channels.
...
The 1-lepton medium-to-high \ptv relative acceptance is based off the value obtained from the 2-lepton channel, since the 1-lepton channel had an insufficient number of events to estimate the uncertainty directly.
...
In order to reduce the effects of statistical fluctuations on the fit, these regions are merged before deriving the shape uncertainty.
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption{
    The comparison of the shapes of the \largeR jet mass \mJ between \SHERPA (black) and \POWHEG{}+\textsc{Pythia8} (red) samples from $WZ$ and $ZZ$ process in high and low purity signal regions, integrated over \pTV regions and the 0- and 1-lepton channels \cite{Dao:2688371}.
...
Two alternate samples were considered: \POWHEG{}+\textsc{Herwig7} (providing a alternate parton shower model) and \textsc{MadGraph5+Pythia8.2} (providing an alternate hard scatter model).
...
Of the acceptance uncertainties, the largest contribution is due to the matrix element calculation, with the parton showering model being second.
...
The dominant process contributing to the single-top background is $\Wboson t$ production, relevant for the 0- and 1-lepton channels.
The same nominal and alternative samples are used as for the \ttbar background.
...
Acceptance and shape uncertainties arising from missing higher-order QCD and electroweak corrections, PDF uncertainties, renormalisation and factorisation scales, and alternate parton showering model are applied.
...
The corresponding post-fit background normalisations are listed in \cref{tab:bkg_norms}.
...
In addition to the observed significance and signal strength, it is also necessary to study the post-fit \mJ distributions to compare level of the agreement between the simulation (using the best-fit values of the signal strength $\hat{\mu}$ and the NP $\hat{\theta}$) and the data.
...
% The only point was, in the text where you explain the experimental uncertainties, make sure they are all described (1-2 lines) and have a heading that links back to this table. That way you make sure they have all been introduced and can be easily linked. A quick/easy way to do it is \begin{description} \item[b-tagging] etc.

\todo{Perhaps add the STXS measurment (figure and table), which is a key result from this week}
\todo{I would also consider adding the post-fit yield tables from the auxillary results as well}
...
An improved method of reconstructing the \largeR jet mass, for example by using a machine learning based regression approach, possibly as an additional auxiliary task to \GNN (see \cref{chap:gnn_tagger}), would therefore be expected to reduce the systematic uncertainty on the \muVH measurement.