...
The Higgs candidate is required to have exactly two ghost-assciated\todo{explain ga} and \btagged variable-radius track-jets.
...
This is done so that \toto{explain why ttbar} 
...
Electrons and muons are reconstructed as outlined in \cref{sec:lepton_reco}, and following the approach described in \rcite{HIGG-2018-04}.\todo{tell tim the previsou snalaysis is cited here, no lepton reco info}
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}%
  \hfill
  \caption{Normalised leading \largeR jet mass distribution from \Zboson and \Whf processes in the HP SR of the \zlep channel \cite{Dao:2688371}. The renormalisation scale $\mu_r$ has been varied by a factor of 2 (1up) and 0.5 (1down). An exponential function is fitted to the ratio between the nominal and variation samples.}
...
\begin{figure}[!htbp]
  ...
  from \Vhf process in \pTV inclusive signal regions \cite{Dao:2688371}.
...
\begin{figure}[!htbp]
  ...
  \caption{
    Normalised leading \largeR jet mass distribution from $WZ$ and $ZZ$ process, merged among all the signal regions and lepton channels \cite{Dao:2688371}.
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption{
    The comparison on \mJ shapes between \SHERPA and \POWPYTHIA8 samples from $WZ$ and $ZZ$ process in high and low purity signal regions \cite{Dao:2688371}.
...
The result of this fit can be used to calculate expected (median) significances, which can be compared to their observed values, as is done in \cref{sec:signal_strengh_sigs}.
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    (left) $\SI{250}{\GeV} < \ptv < \SI{400}{\GeV}$ and (right) $\ptv \geq \SI{400}{\GeV}$ \cite{HIGG-2018-52}.
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    and (bottom) the \zlep\ channel and the \olep\ channel for $\ptv > \SI{400}{\GeV}$ \cite{HIGG-2018-52}.
    The background
...
\begin{figure}[!htbp]
  ...
  \caption{
    \mJ distribution in data after subtraction of all backgrounds except for the $WZ$ and $ZZ$ diboson processes \cite{HIGG-2018-52}. 
...
\begin{figure}[!htbp]
  ...
  \caption{
    Signal strength compatibility test between the (3+1)-POI fit (with the three lepton channels splitted) and the default (1+1)-POI fit \cite{HIGG-2018-52}.
    The compatibility of the three channels is evaluated via a $\chi^2$ difference test and results in a p-value of \pct{49}.
  }
...
\begin{figure}[!htbp]
  ...
  \caption{
    Impact of systematic uncertainties on the fitted $VH$ signal-strength parameter \muVH sorted in decreasing order \cite{Dao:2688371}.
...
The impact on each group is calculated by running a fit with all the NPs in the given group fixed to their nominal values.
...
The signal largest group is the data stat uncertainty (0.25), demonstrating that the analysis would benefit from an increased integrated luminosity or improved efficiency to select signal relevant events (recall from \cref{sec:vhbb_selections} the signal efficiency is in the range of \pct{10}).
...
The biggest contribution to the overall uncertainty is the combined statistical uncertainty on the simulated samples (0.09), which contain only a finite number of events.
