...
Section \cref{sec:experimental_uncertainties} briefly outlines the experiemntal uncertainties impacting the analysis.
Detailed modelling studies performed for the analysis are described in \cref{sec:vhbb_modelling}.
...
The Higgs candidate is required to have exactly two ghost-associated (see \cref{sec:jet_reco}) and \btagged variable-radius track-jets ($b$-track-jets).
...
The 0- and 1-lepton channels also make use of a dedicated \ttbar control region for events with one or more additional \btagged \smallR jets outside the \largeR jet, as described in \cref{sec:vhbb_control_region}.
...
The \VHbb signal yield is extracted from a profile likelihood fit to the \largeR jet mass over several signal and control analysis regions, which are described in \cref{sec:vhbb_selections,sec:vhbb_control_region} respectively.
The diboson background \VZbb yield is simultaneously extracted from the fit, and provides a cross check on the $VH$ signal extraction.
The statistical fit model (described henceforth only as ``the fit'') is described in more detail in described in \cref{sec:vhbb_fit}.
...
The 0- and 1-lepton channels make use of reconstructed missing transverse energy (see \cref{sec:missing_Et}).
The presence of neutrinos in the $\Zboson \higgs \rightarrow \nu\nu \bbbar$ and $\Wboson \higgs \rightarrow \ell\nu \bbbar$ signatures can be inferred from a momentum imbalance in the transverse plane.
...
The analysis makes use of \largeR and \smallR EMTopo jets, and also variable-radius track-jets.
The different types of jets are outlined in \cref{sec:jet_reco}.
The \largeR jets are required to satisfy $\pt > \SI{250}{\GeV}$ and $|\eta| < 2.0$, and are used to reconstruct the Higgs boson candidate, while \smallR jets are used in the selection of the analysis region and in the construction of \ETmiss.
The track-jets are used for \btagging and are required to have at least two constituent tracks with $\pt > \SI{500}{\MeV}$ and $|\eta| < 2.5$, and must themselves satisfy $\pt > \SI{10}{\GeV}$ and $|\eta| < 2.5$.
...
As it is a well understood process, the diboson background can be used as a cross-check on the main \Hbb results.
...
The multijet background in this region is generally made up of events with mismeasured jets and events where the isolated leptonic signature has been mimicked by either a jet or electron from a semi-leptonic heavy flavour decay, where the electron has escaped the jet.
...
Data from centre-of-mass energy \come{13} proton-proton collisions at the \LHC recorded over the course of \runtwo (between 2015 and 2018) were used for the analysis.
...
Modelling uncertainties are described in \cref{sec:vhbb_modelling} and arise due to the imperfections in the Monte-Carlo generators used to model the signal and background events.
...
\begin{itemize}
  ...
  \item The \largeR jet energy and mass scales and resolutions. The scales are calibrated as described in \rcite{JETM-2018-02}, and an uncertainty of \pct{2} and \pct{20} is applied for the jet energy and mass resolutions, respectively \cite{STDM-2011-19,ATLAS-CONF-2016-035}.
...
\section{Modelling Uncertainties}\label{sec:vhbb_modelling}
...
Finally, brief descriptions of the modelling uncertainties applied for the \ttbar and single-top backgrounds are given in \cref{sec:ttbar_modelling}, while the signal modelling uncertainties are outlined in \cref{sec:signal_modelling}.
...
PDFs have to be derived from data and are a significant source of uncertainty in analyses of hadronic collisions.
There are three components to the PDF uncertainties: the statistical and systematic errors on the underlying data used to derive the PDFs, the theory which is used to describe them (which is based on some fixed order perturbative QCD expansion), and finally the procedure which is used to extract the PDFs from the data. 
...
$V$+$bb$ is dominant generally accounting for \pct{80} of the events, while the combined $V$+hf accounts for around \pct{90} of the events.
...
For each variation, the ratio of the variation to nominal is calculated, the up and down variations (if present) are symmetrised, and an analytic function is used to parameterise the ratio.
...
The 0- and 1-lepton channels for the \Whf contribution and the 0- and 2-lepton channels for the \Zhf contribution were found to have compatible shapes in \mJ, and so were jointly measured.
...
\subsection{\texorpdfstring{\ttbar}{ttbar} and single-top Modelling}\label{sec:ttbar_modelling}
...
\input{chapters/6.vhbb_boosted/tables/ttbar_accept_uncert.tex}
...
\input{chapters/6.vhbb_boosted/tables/stop_accept_uncert.tex}

\subsection{Signal Modelling}\label{sec:signal_modelling}
...
A fit to this dataset using the nominal values of the NPs from the simulation will recover the input values and is useful for studying the expected constraints on and correlations between the NPs.

Furthermore, a conditional fit to the Asimov dataset can be performed using values of the background NPs which are determined from an unconditional fit to data.
...
The uncertainty on the overall background may be less than the sum of the individual uncertainties due to the correlations between the different background components.
...
\input{chapters/6.vhbb_boosted/tables/postfit_yields.tex}
...
The binning shown in \cref{fig:vhbb_postfit_plots,fig:vhbb_postfit_plots_cr} is the same as that used in the fit.
...
\input{chapters/6.vhbb_boosted/tables/mu_systs_breakdown.tex}
...
This is in contrast to the resolved analysis, which rejected the background-only hypothesis with an observed (expected) significance of 6.7 (6.7) \cite{HIGG-2018-51}.
...
At the time of writing, the results are the most precise measurements of the $WH$ and $ZH$, \Hbb cross sections in the high \ptv regime.
...
An improved method of reconstructing the \largeR jet mass, for example by using a machine learning based regression approach, possibly as an additional auxiliary task to \GNN, could reduce the systematic uncertainty on the \muVH measurement.
