...
VhbbNice as final state is leptonic so clean. Analysis is split into regions
%
\begin{itemize}
    \item 0-lepton
    \item 1-lepton
...
\end{itemize}
%
Events are categorised into the 0-, 1- and 2-lepton channels depending on the number of selected electrons and muons, to target the $ZH \rightarrow vvb\overline{b}$, $WH \rightarrow \ell \nu b \overline{b}$ and $ZH \rightarrow \ell \ell b \overline{b}$ signatures, respectively. In all
channels, events are required to have exactly two b-tagged jets, which form the Higgs boson candidate. At least one b-tagged jet is required to have pT greater than 45 GeV. Events are further split into 2-jet or 3-jet categories depending on whether additional, untagged jets are present. In the 0- and 1-lepton channels, only one such jet is allowed, as the $t \overline{t}$ background is much larger in events with four jets or more. In the 2-lepton channel any number of jets is accepted in the 3-jet category.

MVA is meant to discriminate in data between signal and background. It is trained using MC. The MVA output is cut on to define signal regions.
MVAs are an evolution of cut based techniques to increasing signal discrimination. Discoveries are made by observing an excess of some process. However signal excesses in comparison to the background can be vanishingly small. Comparing the total number of events that occurred at ATLAS without and without Higgs events would be a fractional percent increase, and would be destroyed by systematic and statistical uncertainties. So, we need to look for an excess after selecting for those events which are most likely to be signal. If we understand how much background this selection process allows through, we can quantify the signal gain over the expected background.



\begin{itemize}
  \item \textbf{Signal region:} where you expect Higgs candidates to be. There might be contamination with other processes: the SR will not be 100\% pure. To LO, there are no additional $b$-jets outside the large-R jet that corresponds to the reconstructed Higgs candidate. So, there is a signal region veto (i.e. events are not accepted into the signal region if) for events with additional b-tagged jets in the event.
  %
  \item \textbf{Control region:} there is a $t\overline{t}$ control region. Here, the Higgs candidate will most likely be built from boosted hadronic decay of the top ($t\rightarrow Wb$). An additional b-jet from the other top-quark will decay still somewhere in the event. Obtain a ttbar CR in 0L and 1L by requiring additional b-tags outside the large-R jet.
\end{itemize}
...
%
\input{chapters/6.vhbb_boosted/tables/sources_of_uncertainty}
%
